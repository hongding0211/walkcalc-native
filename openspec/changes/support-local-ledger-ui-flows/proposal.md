...
This change wires the existing UI and store layer to support local-backed groups and records through the same create, edit, delete, archive, balance, search, and settlement paths used by remote-backed groups. The goal is to make local ledger use feel like first-class WalkCalc behavior while preserving the current login screen and authenticated remote behavior until a later launch-routing change.
...
- Preserve the current login-first startup route for this change. Removing or bypassing the login page is explicitly deferred.
...
- No App launch routing change in this proposal.
