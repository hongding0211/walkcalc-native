# native-auth-session-recovery Specification

## Purpose
Define how native authentication stores refreshable credentials, recovers expired access tokens, and clears account state when recovery fails.
...
- **AND** the next unrecoverable auth rejection routes the user to local home
...
- **AND** it routes to local home

### Requirement: Native auth failure clears session and routes to local home
